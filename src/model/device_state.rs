...
impl DeviceState{
    pub (crate) fn parse(record: &Record) -> Result<Self> {
        match record.record_type() {
            RECORD_TYPE_TEST_RECORD => Ok(Self::TestRecord(TestRecordState::parse(&record.decrypt()?)?)),
            RECORD_TYPE_SOLAR_CHARGER => Ok(Self::SolarCharger(SolarChargerState::parse(&record.decrypt()?)?)),
            RECORD_TYPE_BATTERY_MONITOR => Ok(Self::BatteryMonitor(BatteryMonitorState::parse(&record.decrypt()?)?)),
